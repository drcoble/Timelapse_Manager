...
5. (slow) A tiny real AV1 encode: 3 frames at 64x48, skipped when libsvtav1 is
   not compiled into the installed ffmpeg.
"""
...
def _frame_refs(count: int = 3) -> list[FrameRef]:
    ...
    return [
        FrameRef(
            ...
            absolute_path=_FIXTURES / f"frame_{i:03d}.jpg",
            width=64,
            height=48,
        )
...
def _av1_mkv_spec(output_path: Path, render_root: Path) -> RenderSpec:
    return RenderSpec(
        project_id=1,
        frames=FrameSequence(project_id=1, frames=_frame_refs(3)),
...
def _libsvtav1_available() -> bool:
    """Return True only when ffmpeg and ffprobe exist and libsvtav1 is compiled in."""
...
async def test_real_av1_encode_produces_valid_mkv(tmp_path: Path) -> None:
    """3-frame 64x48 AV1+MKV encode succeeds and ffprobe confirms codec=av1.

    Skipped when libsvtav1 is not compiled into the installed ffmpeg.
    """
